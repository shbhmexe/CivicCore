'use client';

import Link from 'next/link';
import { ShieldCheck, Github, Twitter, Mail } from 'lucide-react';
import { usePathname } from 'next/navigation';

export function Footer() {
    return (
        <footer className="w-full py-12 px-4 border-t border-[#001f3f] bg-[#002f5a] text-white">
            <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
                <div className="space-y-4">
                    <div className="flex items-center space-x-2">
                        <ShieldCheck className="w-8 h-8 text-orange-400" />
                        <span className="text-2xl font-bold text-white">CivicCore</span>
                    </div>
                    <p className="text-blue-100 text-sm leading-relaxed">
                        Empowering citizens to take charge of their urban infrastructure through modern technology and community collaboration.
                    </p>
                    <div className="flex space-x-4 pt-2">
                        <Link href="#" className="p-2 bg-white/10 rounded-full hover:bg-white/20 transition-colors text-blue-100 hover:text-white">
                            <Twitter className="w-5 h-5" />
                        </Link>
                        <Link href="#" className="p-2 bg-white/10 rounded-full hover:bg-white/20 transition-colors text-blue-100 hover:text-white">
                            <Github className="w-5 h-5" />
                        </Link>
                        <Link href="#" className="p-2 bg-white/10 rounded-full hover:bg-white/20 transition-colors text-blue-100 hover:text-white">
                            <Mail className="w-5 h-5" />
                        </Link>
                    </div>
                </div>

                <div>
                    <h4 className="text-white font-semibold mb-6">Platform</h4>
                    <ul className="space-y-4 text-sm text-blue-100">
                        <li><Link href="/dashboard" className="hover:text-orange-400 transition-colors">Incident Feed</Link></li>
                        <li><Link href="/report" className="hover:text-orange-400 transition-colors">Report Issue</Link></li>
                        <li><Link href="#" className="hover:text-orange-400 transition-colors">Karma Leaderboard</Link></li>
                        <li><Link href="/admin" className="hover:text-orange-400 transition-colors font-semibold text-teal-300">Admin Dashboard</Link></li>
                    </ul>
                </div>

                <div>
                    <h4 className="text-white font-semibold mb-6">Resources</h4>
                    <ul className="space-y-4 text-sm text-blue-100">
                        <li><Link href="#" className="hover:text-orange-400 transition-colors">Documentation</Link></li>
                        <li><Link href="#" className="hover:text-orange-400 transition-colors">API Access</Link></li>
                        <li><Link href="#" className="hover:text-orange-400 transition-colors">Privacy Policy</Link></li>
                        <li><Link href="#" className="hover:text-orange-400 transition-colors">Terms of Service</Link></li>
                    </ul>
                </div>

            </div>
            <div className="max-w-7xl mx-auto mt-12 pt-8 border-t border-white/10 text-center text-xs text-blue-200/60 font-medium">
                <p>&copy; {new Date().getFullYear()} CivicCore. Built for a better future.</p>
            </div>
        </footer>
    );
}
